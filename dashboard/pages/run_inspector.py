...
def render(project_dir):
    ...
    render_page_description(
        ...
        "five tabs: Trace Timeline shows every step the engine took (with model, "
        "cache hits, and timing). Evidence displays test results with exit codes "
        "and output. Decisions shows any gate choices made during the run. "
        "Artifacts lets you read the generated architecture, implementation, "
        "and verification docs. Config Snapshot shows the exact settings used."
    )
    ...
    # Pipeline visual for this run — with evidence for accurate pass/fail
    ...
    tab_trace, tab_evidence, tab_decisions, tab_artifacts, tab_config = st.tabs(
        ["Trace Timeline", "Evidence", "Decisions", "Artifacts", "Config Snapshot"]
    )
