...
def _render_tier_card(tier_name, tiers, estimate, descriptions):
    ...
    border_color = "#3498DB" if is_premium else "#27AE60"
    badge = "FULL" if is_premium else "LEAN"
    badge_bg = "#3498DB" if is_premium else "#27AE60"
        ...
        f"""<div style="border:2px solid {border_color}; border-radius:12px;
            ...
            <div style="display:flex; align-items:center; gap:8px; margin-bottom:12px;">
                <span style="background:{badge_bg}; color:white; padding:2px 10px;
                    border-radius:12px; font-size:11px; font-weight:700;">{badge}</span>
                <span style="font-size:18px; font-weight:700; color:#2C3E50;">
                    {desc['label']}</span>
            </div>
            <div style="font-size:13px; color:#5D6D7E; margin-bottom:16px;">
                {desc['summary']}</div>
...
def _render_tier_details(tier_name, descriptions):
    ...
    for item in desc.get("includes", []):
        st.markdown(f"&ensp; ✅ &ensp;{item}")
    for item in desc.get("trade_offs", []):
        st.markdown(f"&ensp; ⚠️ &ensp;{item}")
...
def _render_cost_estimate(project_dir):
    ...
    st.markdown(
        f"""<div style="background:#F0F3F8; border-radius:8px; padding:12px 16px;
...
def render(project_dir):
    ...
    render_page_description(
        "Launch and monitor the autonomous build pipeline. Click <strong>Start Pipeline</strong> "
        "to see a cost estimate and choose between <strong>Premium</strong> (full output, higher cost) "
        "or <strong>MVP</strong> (lean output, lower cost). Once running, the <strong>Progress</strong> "
        "section shows live stage completion and a trace timeline. Stages go through: "
        "Bootstrap → Design → Implement → Extract → Test → Verify. "
        "Check <strong>Recent Runs</strong> at the bottom for past executions."
    )
    ...
    if st.session_state.get("show_tier_selection") and not is_running:
        ...
        if selected:
            # Launch pipeline with chosen tier
        ...
        # Show the start button (or status)
        ...
        with col_btn:
            ...
            if st.button(
                ...
                # Clear stale estimate so it recomputes with latest inputs
                st.session_state.pop("cost_estimate", None)
                st.session_state.pop("cost_tiers", None)
                st.rerun()
